...
include 'dbc.php';
$pageTitle = "Add a new Delivery Boy";
$pageName    = "Delivery Boy";
$linkName    = "Show All Delivery Boy";
$editMode    =   "editData";
$addMode    =   "addData";
$tableName   = $tblDeliveryBoy;
$listPageUrl = $urlDeliveryBoyList;
$imageUrl = $mainPath;
                            ...
                            <form id="InsertUpdateForm" role="form" action="" method="post"
                                ...
                                <div class="form-group row">
                                    <div class="col-xs-12 col-sm-12">
                                        ...
                                        <textarea name="address" class="form-control editor"><?php echo (isset($_GET['id'])) ? $serviceData['address'] : "" ?></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-xs-12 col-sm-12">
                                        <label>Group</label>
                                        <select name="group_id[]" class="form-control js-source-states-2" multiple="multiple" style="width: 100%">
                                            <option value="" disabled>Select Group</option>
                                            <?php
                                            // Assuming $propertyData['key_feature'] contains the comma-separated string of selected key features
                                            $saved_group = isset($serviceData['group_id']) ? explode(',', $serviceData['group_id']) : [];

                                            // Query to fetch all key features
                                            $group_que = "SELECT * FROM tbl_group";
                                            $group_res = mysqli_query($conn, $group_que);
                                            while ($group_row = mysqli_fetch_array($group_res)) {
                                                // Check if the current key feature is in the saved array
                                                $selected = in_array($group_row['id'], $saved_group) ? 'selected' : '';
                                            ?>
